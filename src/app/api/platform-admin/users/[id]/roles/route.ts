...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const incoming = Array.isArray(body.roles) ? body.roles.filter((r: string) => ALLOWED.includes(r)) : [];
  const nextRoles = incoming.length ? Array.from(new Set(incoming)) : ['uploader'];
