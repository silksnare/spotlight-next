...
function isAllowedRole(value: unknown): value is AllowedRole {
  return typeof value === 'string' && ALLOWED.includes(value as AllowedRole)
}
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const incoming: AllowedRole[] = Array.isArray(body.roles) ? body.roles.filter(isAllowedRole) : []
  const nextRoles: AllowedRole[] = incoming.length ? Array.from(new Set(incoming)) : ['uploader']
